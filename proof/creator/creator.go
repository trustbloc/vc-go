...
type cryptographicSigner interface {
	...
	Sign(data []byte, keyType kms.KeyType) ([]byte, error)
}

type creatorOpt func(c *ProofCreator)

func WithProofType(proofDesc proofdesc.LDProofDescriptor, cryptographicSigner cryptographicSigner) creatorOpt {
...
func WithJWTAlg(proofDesc proofdesc.JWTProofDescriptor, cryptographicSigner cryptographicSigner) creatorOpt {
	return func(c *ProofCreator) {
...
func New(opts ...creatorOpt) *ProofCreator {
	c := &ProofCreator{}
...
func (c *ProofCreator) SignLinkedDocument(proof *proof.Proof, keyType kms.KeyType, doc []byte) ([]byte, error) {
	...
	return supportedProof.cryptographicSigner.Sign(doc, keyType)
}
...
func (c *ProofCreator) SignJWT(params jwt.SignParameters, data []byte) ([]byte, error) {
	...
	keyType := supportedProof.proofDescriptor.SupportedVerificationMethods()[0].KMSKeyType

	return supportedProof.cryptographicSigner.Sign(data, keyType)
}
