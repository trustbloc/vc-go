...
func TestMarshalWithDisclosure(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		t.Run("disclose selected claims by creating SD-JWT from vc", func(t *testing.T) {
			...
			resultCred, err := vc.MarshalWithDisclosure(
				...
				DisclosureSigner(testsupport.NewEd25519Signer(privKey), "did:example:abc123#key-1"),
	...
	t.Run("failure", func(t *testing.T) {
		...
		t.Run("missing ProofCreator when creating fresh SD-JWT credential", func(t *testing.T) {
			vc, err := parseTestCredential(t, []byte(jwtTestCredential), WithDisabledProofCheck())
			require.NoError(t, err)
			...
			require.Error(t, err)
			require.Empty(t, resultCred)
			require.Contains(t, err.Error(), "credential needs ProofCreator")
		})

		t.Run("ProofCreator error creating fresh SD-JWT credential", func(t *testing.T) {
			expectErr := fmt.Errorf("expected error")
...
func TestMakeSDJWT(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		t.Run("with default hash", func(t *testing.T) {
			...
			sdjwt, err := vc.MakeSDJWT(testsupport.NewEd25519Signer(privKey), "did:example:abc123#key-1")
			require.NoError(t, err)
		...
		t.Run("with SD JWT V5", func(t *testing.T) {
			...
			sdjwt, err := vc.MakeSDJWT(
				testsupport.NewEd25519Signer(privKey), "did:example:abc123#key-1",
				MakeSDJWTWithVersion(common.SDJWTVersionV5),
	...
	t.Run("failure", func(t *testing.T) {
		t.Run("prepare claims", func(t *testing.T) {
			...
			sdjwt, err := badVC.MakeSDJWT(testsupport.NewEd25519Signer(privKey), "did:example:abc123#key-1")
			require.Error(t, err)
